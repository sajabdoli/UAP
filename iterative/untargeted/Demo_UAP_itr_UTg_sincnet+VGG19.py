...
# Attacking scenario is Untargeted attack. The target model is SincNet+VGG19
...
from UAP_it_Untarg import un_pert_gen
...
if __name__ == '__main__':
    ...
    #LOAD SAVED UrbanSound8k DATASET dumped in .npy files in data folder (8732 audio samples)
    ...
    attacker = DDN_tf(model_end,  batch_shape, steps=50, targeted=False, quantize=True, init_norm=0.2)

    print ("attacking untargeted:")

    # Call UAP generation function
    v, fooling_rate = un_pert_gen(model_end, X, Y, attacker, sess, xi,delta)
    print ("fooling_rate_train", fooling_rate)

    # validation on test set
    SNR_avg, fooling_rate_test=validation(v, model_end, X_test)
    print ("SNR", SNR_avg)
    print ("fooling rate",fooling_rate_test)

    # save file of UAP on disk
    str1="universal-itr-UNTARGETED-sincnet+VGG19.best.npy"
    np.save(str1, v)
